...
int cas=0;
int test;
double a,b,num1,num2,val1,val2;
void  binary_searc()
{
    double  low=0.0,hi=400.0,mid,chap;
            mid=(hi+low)/2.0;
            int i=200;
    while(hi-low>.000000001)
    {
        mid=(hi+low)/2.0;
        double width=(mid*num2)/num1;
        double A=sqrt(width*width+mid*mid)/2.0;
        double C=A;
        double theta=acos(((A*A+C*C)-(width*width))/(2*A*C));
        chap=theta*A;
        if((chap*2+mid*2)<400.0)
        {
            low=mid;
    val1=mid;
        }
