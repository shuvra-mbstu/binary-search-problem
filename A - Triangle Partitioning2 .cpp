...
int test,cas=0;
double rt1,rt2;
double AB,AC,BC,rati,val,area,ans;
void binary_s(){
double low=0.001;
double hi=AB;
double AE,DE,S,area2;
while((hi-low)>0.0000006){
double mid=(low+hi)/2;
AE=(mid/rt1);
DE=(mid/rt2);
S=(mid+DE+AE)/2;
area2=sqrt(S*(S-mid)*(S-AE)*(S-DE));
if((area2/(area-area2))>rati){
     hi=mid;
     ans=mid;
   //  cout<<hi<<endl;
}
...
while(test--){
     cin>>AB>>AC>>BC>>rati;
     val=(AB+BC+AC)/2;
     area=sqrt(val*(val-AB)*(val-BC)*(val-AC));
     rt1=AB/BC;
     rt2=(AB/AC);
     binary_s();
     //cout<<ans<<endl;
