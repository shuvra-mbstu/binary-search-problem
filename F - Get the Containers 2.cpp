...
*/
long long int test, num, arr[1007],cas=0,ves;
long long int ans,sum;
bool fun(long long int xx){
  //   cout<<xx<<endl;
     long long int s=0,x,coun=0;
FOR(i,0,num){
   if((s+arr[i])<=xx){
     s+=arr[i];
     if(i==0)coun++;
     ...
     if(arr[i]>xx || (coun+1)>ves)return false;

  //cout<<arr[i]<<" "<<coun+1<<endl;
     s=arr[i];
     coun++;
   }
}
